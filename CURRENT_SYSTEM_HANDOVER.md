...
- Streamlined UI & Manual Sync Removal (v2.8.17): Completely removed the manual "Sync Cloud Data" (`onForceSync`) button from DashboardView and Sidebar. Because Firestore Listeners and Live Relay synchronize all updates in < 100ms automatically, manual syncing was redundant and risked 26-second tab stalls for members. Manual backup/sync remains securely accessible inside the Owner-only GoogleDriveBackupModal.
- Firebase Firestore Primary & Dual-Write Architecture (v2.8.17): Migrated to Firebase Firestore as the 100% Primary Database for all core collections (users, items, item_queues, clans, diamond_vault, quick_items, general_items). Populated 49 items, 7 queues, 24 members, and settings into Firestore Enterprise DB. Deployed updated firestore.rules for instant (< 200ms) unauthenticated reads while guarding admin/owner writes. Maintained 100% background Dual-Write to Google Sheets & Drive (10s debounce) and automated sync back to Firestore on quota recovery.
- Relay-Streamlined Synchronization (v2.8.17): Removed the legacy 35-second periodic Google Sheets fallback polling interval (setInterval) from googleSheetsBackupService.ts. Real-time multi-device synchronization is handled 100% via Live Relay Server long-polling (< 50ms latency), saving ~1.75 MB payload downloads every 35s per tab, eliminating UI stutter, and protecting Google Apps Script execution quotas while preserving Google Drive snapshot backups on state changes.
- Cloud-First Architecture & Tier 1/2 Elimination (v2.8.14): Eliminated hardcoded mock member files (3,200 lines / ~87KB in offlineMembersFallback.ts) and removed local collection storage caching in LocalStorage. The client operates in a 100% Cloud-First mode hydrating directly from Google Sheets & Live Relay Server, while preserving login session (l2m_user_session) and deletion/unclaim tombstones (syncMeta).
